...
#define PERIOD 100000 // Microsecond
...
void loop() {
  if(Serial.available() != 0){ // Wait for data available
    ...
    //   input = message.substring(commaIndex + 1);
    ...
    for(int i = 15; i >= 0; i--){
      ...
      bin.push_back(bitRead(res, i));
      Serial.println(bitRead(res, i));
       // Add CRC bits to bin bit vector.
    }
    Serial.println(bin.size());
    // Serial.println(bin);
    tx_ready = true;
